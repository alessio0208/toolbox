...
options:
   ...
   -hs-features { YES | NO } : Generates 6 additional features used for 
                               evaluating hidden services
   -skip-cum-features { YES | NO } : Skip the First 50 Interpolated Features.
								Used for hidden services, CW Scenario.

Default Configuration is extracted from Environmental Variables
        Check, Adjust & Reload WFP_config if necessary
        
Hidden Service Features: 6 new features are added: 
	number of used entry nodes, 
	abs packet size of the most used entry_1, 
	abs packet size of the most used entry_2,
	abs packet size of the most used entry_3, 
	abs packet size of the most used entry_4,
	abs packet size of the most used entry_5, where
	abs packet size of the most used entry_i >= abs packet size of the most used entry_(i-1).
...
tmp4 = 'NO' # Option "-force" is disabled by default
tmp6 = 'NO' # Option "-hs-features" is disabled by default
tmp7 = 'NO' # Option "-skip-cum-features" is disabled by default
...
if len(sys.argv) != 0:
    ...
    while i < len(options):
        if options[i] == '-in':
            ...
            i = i + 1
            tmp5 = options[i]
        elif options[i] == '-hs-features':
            i = i + 1
            tmp6 = options[i]
        elif options[i] == '-skip-cum-features':
            i = i + 1
            tmp7 = options[i]
        else:
...
if tmp6 in [ 'YES', 'NO' ]:
    if tmp6 == 'YES':
        generate_hs_features = True
    else:
        generate_hs_features = False
else:
    exit_with_help('Error: Unknown Generate-hs-Features Option!')
if tmp7 in [ 'YES', 'NO' ]:
    if tmp7 == 'YES':
        skip_cum_features = True
    else:
        skip_cum_features = False
else:
    exit_with_help('Error: Unknown Skip-cum-Features Option!')
    ...
    for inFile in inFiles:
        ...
        for _, instance in samples:
            ...
            for item in itertools.islice(instance.packets, None): 
                ...
                if generate_hs_features:
                    found = False
                    if len(entry_node_packets) > 0:
                        for entry in entry_node_packets:
                            if entry.entry == item.torip:
                                entry.packets.append(packetsize)
                                found = True
                    if not found:
                        firstentry = EntryNodePacket(item.torip)
                        firstentry.packets.append(packetsize)
                        entry_node_packets.append(firstentry)
            ...
            # Six additional features for hidden services
            if generate_hs_features:
                # Number of entry nodes used
			    features.append(instance.entries)
            
                # Abs packet size per entry node
			    abs_entry_node_packets = []
			    for entry in entry_node_packets:
				    abspacketsize = 0
				    for packet in entry.packets:
					    abspacketsize += abs(packet)
				    abs_entry_node_packets.append(abspacketsize)
				
			    for size in sorted(abs_entry_node_packets):
				    features.append(size)
				
			    if len(abs_entry_node_packets) < 5:
				    rest_entries = 0
				    while rest_entries < 5 - len(abs_entry_node_packets):
					    features.append('0')
					    rest_entries += 1
            ...
            if separateClassifier:
                ...
                if skip_cum_features:
                    for el in itertools.islice(cumFeatures, 51, None):
                        features.append(el)
                else:
...
for dirNumber in range(0,len(formats)):
    ...
    if countList[dirNumber]-1 != -1:
        print('INFO: (' + str(countList[dirNumber]-1) + '/' + str(maxList[dirNumber]) + ') ' +  formats[dirNumber][1] + ' - Format')
    elif dataSet == '_':
        try:
            os.rmdir(featurePath + 'feature-' + formats[dirNumber][0])
        except OSError as e:
            if e.errno != errno.ENOENT:
                raise
