...
if os.path.isdir(wangPath + 'output-cell-wang-legacy/'):
    shutil.rmtree(wangPath + 'output-cell-wang-legacy/')
os.mkdir(wangPath + 'output-cell-wang-legacy/')

if os.path.isdir(wangPath + 'output-cell-nosendme-wang-legacy/'):
    shutil.rmtree(wangPath + 'output-cell-nosendme-wang-legacy/')
os.mkdir(wangPath + 'output-cell-nosendme-wang-legacy/')


tlsfiles = natsorted(glob.glob(outlierfreePath + 'output-tls-legacy-outlierfree/*'))

# Create Matching file
matchingFile = open(wangPath + 'Matching-legacy.txt', 'w')
...
for tlsfile in tlsfiles:
    ...
    count=0
    # Read file
    for line in open(outlierfreePath + 'output-tls-legacy-outlierfree/' + inputfilename, 'r'):
        entries = []
        ...
        for entry in entries[2:]:
            ...
            data.append((int(tsinfo), int(tls)))
        ...
        # Outputfile
        if setting == 'OW_BG':
            cellfileWangSend = open(wangPath + 'output-cell-wang-legacy/' + str(page), 'w')
            cellfileWangNoSend = open(wangPath + 'output-cell-nosendme-wang-legacy/' + str(page), 'w')
        else:
            cellfileWangSend = open(wangPath + 'output-cell-wang-legacy/' + str(page) + '-' + str(count), 'w')
            cellfileWangNoSend = open(wangPath + 'output-cell-nosendme-wang-legacy/' + str(page) + '-' + str(count), 'w')
