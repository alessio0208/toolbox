...
def outputInput():
	for currentFold in range(1, folds+1):
		...
		ftrainout = open(scenario + '_' + str(currentFold) + '.train', 'w')
		ftestout = open(scenario + '_' + str(currentFold) + '.test', 'w')
...
def evaluation():
	...
	for currentFold in range(1, folds+1):
		cmd = '{0} -v {1} -svmtrain "{2}" -gnuplot "{3}" "{4}"'.format(grid_py, folds, svmtrain_exe_q, gnuplot_exe, scenario + '_' + str(currentFold) + '.train')
		print('Fold: ' + str(currentFold) + ' - Cross validation...')
		...
		print(' Fold: ' + str(currentFold) + ' - Best c={0}, g={1} CV rate={2}'.format(results[currentFold-1][0],results[currentFold-1][1],results[currentFold-1][2]))

	# train model for each fold
	for currentFold in range(1, folds+1):
		cmd = '{0} -c {1} -g {2} "{3}" "{4}"'.format(svmtrain_exe_q,results[currentFold-1][0],results[currentFold-1][1],scenario + '_' + str(currentFold) + '.train', scenario + '_' + str(currentFold) + '.model')
		print('Fold: ' + str(currentFold) + ' - Training...')
		Popen(cmd, shell = True, stdout = PIPE).communicate()

	# test model for each fold of each fg class
	for currentFold in range(1, folds+1):
		cmd = '{0} "{1}" "{2}" "{3}"'.format(svmpredict_exe, scenario + '_' + str(currentFold) + '.test', scenario + '_' + str(currentFold) + '.model', scenario + '_' + str(currentFold) + '.predict')
		print('Fold: ' + str(currentFold) + ' - Testing...')
		Popen(cmd, shell = True).communicate()

	# merge result for each fold
	fresult = open(scenario + '.result', 'w')
	for currentFold in range(1, folds+1):
		fpredict = open(scenario + '_' + str(currentFold) + '.predict', 'r')
		for line in fpredict:
			fresult.write(line)
		fpredict.close()
	fresult.close()
	
	print('Output prediction: {0}'.format(scenario + '.result'))
	
	return

def removeTemp():
	for currentFold in range(1, folds+1):
		os.remove(scenario + '_' + str(currentFold) + '.train')
		os.remove(scenario + '_' + str(currentFold) + '.test')
		os.remove(scenario + '_' + str(currentFold) + '.predict')
